/**
  @file src/loaders/loaders_linux/st_base_component_loader.c

  ST specific component loader for local components. This veriosn is dinalically loaded
	in the file .omxloaders is present and the library of this laoder is present in the registry
...
#include "common.h"
#include "st_base_component_loader.h"
...
void setup_component_loader(BOSA_COMPONENTLOADER * st_base_loader) {
	  ...
	  st_base_loader->BOSA_DestroyComponent = &BOSA_ST_DestroyComponent;
	  st_base_loader->BOSA_ComponentNameEnum = &BOSA_ST_ComponentNameEnum;
	  st_base_loader->BOSA_GetRolesOfComponent = &BOSA_ST_GetRolesOfComponent;
	  st_base_loader->BOSA_GetComponentsOfRole = &BOSA_ST_GetComponentsOfRole;

}

/** @brief the ST base loader contructor
 *
 * This function creates the ST base component loader, and creates
    ...
    *(line+index) = 0;
    ...
    if((handle = dlopen(libname, RTLD_NOW)) == NULL) {
      ...
      if ((fptr = dlsym(handle, "omx_component_library_Setup")) == NULL) {
        DEBUG(DEB_LEV_ERR, "the library %s is not compatible with ST base component loader - %s\n", libname, dlerror());
      } else {
...
OMX_ERRORTYPE BOSA_ST_CreateComponent(
  ...
  if (componentPosition == -1) {
    DEBUG(DEB_LEV_ERR, "Component not found with current ST base component loader.\n");
 ...
 *
 * This function searches in the list of ST base components and enumerates
 ...
 * standard OMX_GetRolesOfComponent function for the ST base
 * component loader
 */
 ...
 *
 * This function replicates exactly the behavior of the
 * standard OMX_GetComponentsOfRole function for the ST base
 * component loader
 */
