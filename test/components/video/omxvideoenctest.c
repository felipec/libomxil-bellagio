/**
  ...
  The application receives an video stream (.yuv) encode by a multiple format(.m4v or .h264) encoder component.
...
OMX_ERRORTYPE test_OpenClose(OMX_STRING component_name) {
  ...
  DEBUG(DEFAULT_MESSAGES, "GENERAL TEST %s\n",__func__);
  err = OMX_GetHandle(&appPriv->videoenchandle, component_name, NULL /*appPriv */, &videoenccallbacks);
  if(err != OMX_ErrorNone) {
...
int find_encoder(char* searchname) {
  ...
  if (strstr(filename_lower, "m4v") != NULL) {
    ...
    return 0;
  } else if (strstr(filename_lower, "h264") != NULL) {
    strcpy(&codecName[0],"h264");
    return 0;
  } 
  return 1;
}
...
void display_help() {
  ...
  printf("                     the file name looks like input_filename_app.m4v/.h264 depending on input option\n");
  printf("\n");
  ...
  if(argc < 2){
    ...
    while (argn_dec < argc) {
      if (*(argv[argn_dec]) == '-') {
        ...
        if (flagIsOutputEspected) {
          if(strstr(argv[argn_dec], ".m4v") == NULL && strstr(argv[argn_dec], ".264") == NULL) {
          ...
          if(strstr(argv[argn_dec], "m4v") != NULL) {
            strcpy(&codecName[0],"m4v");
          } else if(strstr(argv[argn_dec], "h264") != NULL) {
            strcpy(&codecName[0],"h264");
          }
          else {
    ...
    if ((!flagOutputReceived) || ((strstr(output_file, ".m4v") == NULL) && (strstr(output_file, ".264") == NULL))) {
      DEBUG(DEB_LEV_ERR, "You must specify appropriate input file of .m4v or .264 format\n");
      display_help();
    }
    ...
    } else if((strstr(output_file, ".264") != NULL)) {
...
OMX_ERRORTYPE videoencEmptyBufferDone(
  ...
  if(!flagIsCameraRequested) {
    ...
    DEBUG(DEB_LEV_ERR, "In %s: Dropping Fill This Buffer\n", __func__);
  }
  return OMX_ErrorNone;
}
